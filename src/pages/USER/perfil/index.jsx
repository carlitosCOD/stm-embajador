...
function FotoPanel({ onUploaded, editMode }) {
  ...
  const handleFileChange = (e) => {
    ...
    // Forzar proporción 1:1
    const img = new Image();
    img.src = URL.createObjectURL(f);
    img.onload = () => {
      const canvas = document.createElement("canvas");
      const size = Math.min(img.width, img.height); // cuadrado más pequeño
        ...
        <div className="card-avatar">
          <label className="avatar-editable" style={{ cursor: "pointer" }}>
            ...
            <img
              ...
              onClick={() => {
                if (!editMode) setShowModal(true); // ✅ Vista previa si no está en edición
              }}
            />
          </label>
        </div>
  ...
  {usuario?.verificado === 1 && <span className="verified">✔️</span>}
</h2>
